...
class RotNetDataGenerator(object):
    ...
    def generate(self, input,labels,num_classes_focal):
        ...
        while 1:
            ...
            for i in range(imax):
                ...
                # Generate data
                # try:
                X, y = self.__data_generation(image_path_temp,labels_temp,num_classes_focal)
    ...
    def __data_generation(self, input,labels,num_classes_focal):
        ...
        for index, current_input in enumerate(input):
            ...
            batch_label_dist.append(labels[index])
            batch_input_focal.append(input_focal)
        batch_input_focal = np.array(batch_input_focal)
        batch_label_dist = np.array(batch_label_dist)
        # preprocess input images
        if self.preprocess_func:
            batch_x = self.preprocess_func(batch_x)

            return {'main_input': batch_x, 'concat_input': batch_input_focal}, batch_label_dist
