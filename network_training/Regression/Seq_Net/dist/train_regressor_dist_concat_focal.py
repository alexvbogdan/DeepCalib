...
from utils_regressor_dist_concat_focal import RotNetDataGenerator, angle_error, CustomModelCheckpoint
...
def get_paths(IMAGE_FILE_PATH_DISTORTED):
    ...
    paths_valid = glob.glob(IMAGE_FILE_PATH_DISTORTED + "*.jpg")
    paths_valid.sort()
    parameters = []
    labels_focal_valid = []
...
with tf.device('/gpu:0'):
    ...
    final_output_dist = Dense(1, activation='sigmoid', name='output_focal')(phi_concat)

    model = Model(input=[image_input,concat_input], output= final_output_dist)
    ...
    generator_training = RotNetDataGenerator(input_shape=image_shape, batch_size=batch_size, one_hot=True,
                                             preprocess_func=preprocess_input, shuffle=True).generate(input_train,
                                                                                                      labels_train,len(classes_focal))
    generator_valid = RotNetDataGenerator(input_shape=image_shape, batch_size=batch_size, one_hot=True,
                                          preprocess_func=preprocess_input, shuffle=True).generate(input_valid,
                                                                                                   labels_valid,len(classes_focal))
