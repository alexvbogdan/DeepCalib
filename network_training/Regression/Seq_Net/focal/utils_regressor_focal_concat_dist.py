...
class RotNetDataGenerator(object):
    ...
    def generate(self, input,labels,num_classes_distortion):
        """
        Generate images.
        ...
        Args:
            ...
            num_classes_distortion: (int): write your description
        """
        # Infinite loop
        while 1:
            # Generate order of exploration of dataset
            if self.shuffle:
                c = list(zip(input, labels))
                random.shuffle(c)
                image_path, labels = zip(*c)
            # Generate batches
            imax = int(len(input)/self.batch_size)
            for i in range(imax):
                start_index = i * self.batch_size
                end_index = (i+1) * self.batch_size
                image_path_temp = input[start_index:end_index]
                labels_temp = labels[start_index:end_index]
                # Generate data
                # try:
                X, y = self.__data_generation(image_path_temp,labels_temp,num_classes_distortion)
    ...
    def __data_generation(self, input,labels,num_classes_distortion):
        """
        Data generation of the image.
        ...
        for index, current_input in enumerate(input):
            is_color = int(self.color_mode == 'rgb')
            image = cv2.imread(current_input[0], is_color)
            if is_color:
            ...
            batch_x[index] = image
            # if self.one_hot:
            input_dist = to_categorical(int(current_input[1]), num_classes_distortion)
            batch_label_focal.append(labels[index])
            batch_input_dist.append(input_dist)
        batch_label_focal = np.array(batch_label_focal)
        batch_input_dist = np.array(batch_input_dist)
        # preprocess input images
        if self.preprocess_func:
            batch_x = self.preprocess_func(batch_x)

            return {'main_input': batch_x, 'concat_input': batch_input_dist}, batch_label_focal
