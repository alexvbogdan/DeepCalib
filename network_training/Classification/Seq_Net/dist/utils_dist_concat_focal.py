...
def crop_image(image):
    """
    Crops an image to a given width.
...
class RotNetDataGenerator(object):
    ...
    def generate(self, input,labels,num_classes_focal,num_classes_distortion):
        """
        Generate images.
    ...
    def __data_generation(self, input,labels,num_classes_focal,num_classes_distortion):
        """
        Data generation.
        ...
        for index, current_input in enumerate(input):
            ...
            batch_x[index] = image
            # if self.one_hot:
            input_focal = to_categorical(int(current_input[1]), num_classes_focal)
            label_distortion = to_categorical(int(labels[index]),num_classes_distortion)
            batch_label_focal.append(input_focal)
        ...
        return {'main_input':batch_x, 'concat_input':batch_label_focal}, batch_label_distortion
