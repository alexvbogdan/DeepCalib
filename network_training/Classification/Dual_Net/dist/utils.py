...
class RotNetDataGenerator(object):
    ...
    def generate(self, image_path,labels,num_classes):
        """
        Generate images.
        ...
        while 1:
            ...
            for i in range(imax):
                ...
                # Generate data
                # try:
                X, y = self.__data_generation(image_path_temp,labels_temp,num_classes)
    ...
    def __data_generation(self, image_path_temp,labels,num_classes):
        """
        Preprocess image.

        Args:
            self: (todo): write your description
            image_path_temp: (str): write your description
            labels: (str): write your description
            num_classes: (int): write your description
        """
        ...
        for index, current_path in enumerate(image_path_temp):
            ...
            image = image * 2.
            batch_x[index] = image
            batch_label[index] = int(labels[index])



        if self.one_hot:
        # convert the numerical labels to binary labels
            batch_label = to_categorical(batch_label, num_classes)
