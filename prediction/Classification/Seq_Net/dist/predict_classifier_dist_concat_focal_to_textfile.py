...
def get_paths(IMAGE_FILE_PATH_DISTORTED):
    """
    Returns a list of paths for a list of paths.
...
with tf.device('/gpu:1'):
    ...
    for i, curr_input in enumerate(input_test):
        ...
        prediction_dist = model.predict({'main_input_phi':image, 'concat_input':input_focal.reshape((1, -1))})

        n_acc_dist += classes_distortion[np.argmax(prediction_dist[0])]


        file.write(curr_input[0] + '\tlabel_focal\t' + str(classes_focal[curr_input[1]]) + '\tlabel_dist\t' + str(classes_distortion[labels_test[i]]) + '\tprediction_dist\t' + str(classes_distortion[np.argmax(prediction_dist[0])])+'\n')
