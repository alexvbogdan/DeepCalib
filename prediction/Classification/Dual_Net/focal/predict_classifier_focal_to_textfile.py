...
def get_paths(IMAGE_FILE_PATH_DISTORTED):
    """
    Return list of paths of all of paths.
...
with tf.device('/gpu:0'):
    ...
    final_output_phi = Dense(len(classes_focal), activation='softmax', name='fc181-phi')(phi_flattened)
    ...
    for i, path in enumerate(paths_test):
        ...
        # loop
        prediction_focal = model.predict(image)


        n_acc_focal += classes_distortion[np.argmax(prediction_focal[0])]

        file.write(path + '\tprediction_focal\t' + str(classes_focal[np.argmax(prediction_focal[0])]) + '\tlabel_focal\t' + str(classes_focal[labels_test[i][0]]) + '\tlabel_dist\t' + str(classes_distortion[labels_test[i][1]]) + '\n')
